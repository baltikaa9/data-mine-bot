...
from .utils import process_classification
from .utils import process_csv, plot_clusters_count, process_customers, plot_correlation_matrix
...
async def cmd_start(message: Message):
    await message.answer(
        'Привет, это бот для визуализации результатов работы методов МО при решении задач классификации и кластеризации.\n'
        'Выбери действие:',
...
async def select_method_no_clusters(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        reply_markup=get_online_retail(),
    )
...
async def handle_auto_clusters(callback: types.CallbackQuery, state: FSMContext):
    await callback.message.edit_text(
        'Загрузи CSV-файл для анализа оптимального числа кластеров:',
        reply_markup=get_online_retail(),
    )
...
@router.callback_query(Clustering.waiting_for_auto_clusters, F.data.in_(['online_retail']))
async def handle_auto_clusters_file(message: Message, state: FSMContext):
    if not hasattr(message, 'document'):
        file = open('csv/OnlineRetail_rus.csv', 'rb')
...
@router.callback_query(Clustering.waiting_for_file, F.data.in_(['online_retail']))
async def handle_csv(message: Message | CallbackQuery, state: FSMContext):
...
async def select_classification_method(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        "Теперь загрузи CSV-файл с данными",
        reply_markup=get_customer_behavior(),
    )
...
@router.callback_query(Classification.waiting_for_file, F.data.in_(['customer_behavior']))
async def handle_target_column(message: Message | CallbackQuery, state: FSMContext):
    if not hasattr(message, 'document'):
        file = open('csv/Customer_Behaviour_rus.csv', 'rb')
        await state.update_data(customer=True)
        # file_bytes = file.read()
    else:
    # Скачиваем файл
        file = await message.bot.download(message.document)
        await state.update_data(customer=False)

    await state.update_data(file=file)
    if isinstance(message, CallbackQuery):
        await message.message.answer("Введи название целевой колонки")
        await message.answer()
    else:
...
async def handle_classification_file(message: Message, state: FSMContext):
    ...
    if data['customer']:
        image_data, error, accuracy = await process_customers(
            file_bytes,
            target,
            method=method
        )
